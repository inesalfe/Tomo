function save_sparse_ct_poisson_book(N_pixels,theta,seed,intensity)
    ...
    fileID = fopen('ct_poisson_book/params_sparse.txt','a+');
    ...
    fileID_error = fopen('ct_poisson_book/params_sparse.txt','a+');
    ...
    filename_fig = "ct_poisson_book/x_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".png";
    ...
    caxis manual
    caxis([min_pixel max_pixel])
    filename_fig = "ct_poisson_book/x_ls_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".png";
    ...
    caxis manual
    caxis([min_pixel max_pixel])
    filename_fig = "ct_poisson_book/x_ls_pos_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".png";
    ...
    caxis manual
    caxis([min_pixel max_pixel])
    filename_fig = "ct_poisson_book/x_ls_box_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".png";
    ...
    filename_idx_non_zero_pixels = "ct_poisson_book/idx_non_zero_pixels_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
    ...
    filename_is_pixel_zero = "ct_poisson_book/is_pixel_zero_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
    ...
    filename_error = "ct_poisson_book/error_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
    file_error = fopen(filename_error,'w');
    fwrite(file_error, error, 'double');
    fclose(file_error);

    filename_x = "ct_poisson_book/x_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
    file_x = fopen(filename_x,'w');
    fwrite(file_x, x, 'double');
    fclose(file_x);

    filename_x_ls = "ct_poisson_book/x_ls_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
    file_x_ls = fopen(filename_x_ls,'w');
    fwrite(file_x_ls, x_ls, 'double');
    fclose(file_x_ls);

    filename_x_ls_pos = "ct_poisson_book/x_ls_pos_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
    file_x_ls_pos = fopen(filename_x_ls_pos,'w');
    fwrite(file_x_ls_pos, x_ls_pos, 'double');
    fclose(file_x_ls_pos);

    filename_x_ls_box = "ct_poisson_book/x_ls_box_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
    file_x_ls_box = fopen(filename_x_ls_box,'w');
    fwrite(file_x_ls_box, x_ls_box, 'double');
    fclose(file_x_ls_box);
    
    filename_b = "ct_poisson_book/b_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
    file_b = fopen(filename_b,'w');
    fwrite(file_b, b, 'double');
    fclose(file_b);

    filename_b_error = "ct_poisson_book/b_error_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
    file_b_error = fopen(filename_b_error,'w');
    fwrite(file_b_error, b_error, 'double');
    fclose(file_b_error);

    filename_row_idx = "ct_poisson_book/row_idx_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
    file_row_idx = fopen(filename_row_idx,'w');
    fwrite(file_row_idx, row_idx, 'int');
    fclose(file_row_idx);

    filename_cols = "ct_poisson_book/cols_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
    file_cols = fopen(filename_cols,'w');
    fwrite(file_cols, NNZ, 'int');
    fwrite(file_cols, cols, 'int');
    fclose(file_cols);

    filename_values = "ct_poisson_book/values_" + int2str(M) + "_" + int2str(N) + "_" + int2str(seed) + ".bin";
