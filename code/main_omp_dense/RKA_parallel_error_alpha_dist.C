...
int main (int argc, char *argv[]) {
	...
	if(argc != 7 && argc != 8) {
		...
		cout << "'./bin/RKA_parallel_error_alpha_dist.exe <data_set> <n_runs> <eps> <M> <N> <alpha>'" << endl;
		exit(1);
	}
	...
	if (argc == 7 && matrix_type.compare("ct") == 0) {
		...
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/RKA_parallel_error_alpha_dist.exe <data_set> <n_runs> <eps> <M> <N> <alpha> <max_it_stop>'" << endl;
		exit(1);
	}
