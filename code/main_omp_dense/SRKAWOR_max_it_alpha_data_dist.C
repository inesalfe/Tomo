...
int main (int argc, char *argv[]) {
	...
	if(argc != 9 && argc != 10) {
		...
		cout << "'./bin/SRKAWOR_max_it_alpha_data_dist.exe <data_set> <n_runs> <M> <N> <it_per_thread> <alpha> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	string filename_error = "errors/omp_dense/SRKAWOR_alpha_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + ".txt";
	string filename_res = "errors/omp_dense/SRKAWOR_alpha_res_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + ".txt";
