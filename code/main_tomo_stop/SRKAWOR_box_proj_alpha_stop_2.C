...
int main (int argc, char *argv[]) {
	...
	if(argc != 8 && argc != 9) {
		...
		cout << "'./bin/SRKAWOR_box_proj_alpha_stop_2.exe <data_set> <n_runs> <M> <N> <it_per_thread> <bucket_size> <alpha>'" << endl;
		exit(1);
	}
		...
		while(!solution_found) {
			...
			for (int iner_it = 0; iner_it < M/it_per_thread; iner_it++) {
				...
				for (int i = 0; i < it_per_thread; i++) {
					line = samp_line[(it_per_thread*iner_it+i)];
					// cout << line << " " << i << endl;
				...
				for (int i = 0; i < it_per_thread; i++) {
					line = samp_line[M-(it_per_thread*iner_it+i)-1];
					// cout << line << " " << i << endl;
	...
	string filename_sol = "outputs/tomo_stop/" + matrix_type + "/SRKAWOR_box_proj_alpha_sol_2_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + str_alpha;
