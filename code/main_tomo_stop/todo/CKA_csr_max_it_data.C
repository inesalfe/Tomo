...
int main (int argc, char *argv[]) {
	...
	if(argc != 7 && argc != 8) {
		...
		cout << "'./bin/CKA_csr_max_it_data.exe <data_set> <n_runs> <M> <N> <it_per_thread> <bucket_size>'" << endl;
		exit(1);
	}
	...
	int storage_size = ceil(bucket_size/step_save);
	int storage_counter;
	...
	string filename_error = "errors/omp_sparse/" + matrix_type + "/CKA_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + to_string(bucket_size);
	string filename_res = "errors/omp_sparse/" + matrix_type + "/CKA_res_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + to_string(bucket_size);
