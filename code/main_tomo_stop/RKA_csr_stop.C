...
int main (int argc, char *argv[]) {
	...
	if(argc != 7 && argc != 8) {
		...
		cout << "'./bin/RKA_csr_stop.exe <data_set> <n_runs> <M> <N> <it_per_thread> <bucket_size>'" << endl;
		exit(1);
	}
	...
	string filename_sol = "outputs/tomo_stop/" + matrix_type + "/RKA_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread);
