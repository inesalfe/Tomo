...
// ./bin/CKA_box_proj_csr_alpha_stop.exe ct_gaussian 1 19558 16384 3 7 1.1 2

int main (int argc, char *argv[]) {

	if(argc != 8 && argc != 9) {
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/CKA_box_proj_csr_alpha_stop.exe <data_set> <n_runs> <M> <N> <it_per_thread> <bucket_size> <alpha>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		while(!solution_found) {
			...
			for (int iner_it = 0; iner_it < M/it_per_thread; iner_it++) {
				...
				for (int i = 0; i < it_per_thread; i++) {
					line = M-(it_per_thread*iner_it+i)-1;
					// cout << line << " " << i << endl;
	...
	string filename_sol = "outputs/tomo_stop/" + matrix_type + "/CKA_box_proj_alpha_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + str_alpha;
