...
int main (int argc, char *argv[]) {
	...
	if(argc != 6 && argc != 7) {
		...
		cout << "'./bin/SRKAWOR_box_proj_parallel_stop_4.exe <data_set> <n_runs> <M> <N> <bucket_size>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		it = 0;
		start = omp_get_wtime();
		#pragma omp parallel private(line_down, line_up, line_down_index, line_up_index, scale, t_id, iner_it, col, index) firstprivate(it)
		{
			...
			if (t_id < 0) {
				...
				while(!solution_found) {
					it++;
					line_down_index = iner_it%M;
					line_up_index = M-line_down_index-1;
					while (line_down_index < M) {
						line_down = samp_line[line_down_index];
						...
						line_down_index += num_threads;
						line_up = samp_line[line_up_index];
				...
				while(!solution_found) {
					...
					line_up_index = M-line_down_index-1;
					while (line_down_index < M) {
						// if (t_id == 1 && (line_down < 30 || line_down > 19510))
						// 	cout << t_id << " " << it << " " << line_down << " " << line_up << endl;
						line_down = samp_line[line_down_index];
						scale = (b[line_down]-dotProductCSR(line_down, row_idx, cols, values, x_down))/sqrNorm_line[line_down];
						index = row_idx[line_down];
						while (index < row_idx[line_down+1]) {
	...
	string filename_sol = "outputs/tomo_stop/" + matrix_type + "/SRKAWOR_box_proj_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads_total);
