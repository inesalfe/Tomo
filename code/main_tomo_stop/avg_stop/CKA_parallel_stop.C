...
int main (int argc, char *argv[]) {
	...
	if(argc != 6 && argc != 7) {
		...
		cout << "'./bin/CKA_parallel_stop.exe <data_set> <n_runs> <M> <N> <bucket_size>'" << endl;
		exit(1);
	}
	...
	string filename_sol = "outputs/tomo_stop/" + matrix_type + "/CKA_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads_total);
