...
int main (int argc, char *argv[]) {
	...
	if(argc != 6 && argc != 7) {
		...
		cout << "'./bin/RKA_box_proj_parallel_stop_6.exe <data_set> <n_runs> <M> <N> <bucket_size>'" << endl;
		exit(1);
	}
	...
	string filename_sol = "outputs/tomo_stop/" + matrix_type + "/RKA_box_proj_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads);
