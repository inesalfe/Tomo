...
// ./bin/SRKABWOR_csr_stop.exe ct_gaussian 10 19558 16384 1 15000 5000 1 2

int main (int argc, char *argv[]) {

	if(argc != 9 && argc != 10) {
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/SRKABWOR_csr_stop.exe <data_set> <n_runs> <M> <N> <threads> <block_size> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	if (argc == 9 && matrix_type.compare("ct") == 0) {
		...
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/SRKABWOR_twin_csr_max_it_data.exe <data_set> <n_runs> <M> <N> <threads> <block_size> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	string filename_error = "errors/omp_sparse/" + matrix_type + "/SRKABWOR_twin_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(threads) + "_" + to_string(block_size) + "_" + to_string(max_it_stop);
	string filename_gauge = "errors/omp_sparse/" + matrix_type + "/SRKABWOR_twin_gauge_" + to_string(M) + "_" + to_string(N) + "_" + to_string(threads) + "_" + to_string(block_size) + "_" + to_string(max_it_stop);
