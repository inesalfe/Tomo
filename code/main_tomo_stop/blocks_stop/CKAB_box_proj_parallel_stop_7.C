...
int main (int argc, char *argv[]) {
	...
	if(argc != 8 && argc != 9) {
		...
		cout << "'./bin/CKAB_box_proj_parallel_stop_3.exe <data_set> <n_runs> <M> <N> <number_of_blocks> <min_it> <bucket_size>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		it = 0;
		start = omp_get_wtime();
		#pragma omp parallel private(line_down, line_up, scale, t_id, x_down_thread, x_up_thread) firstprivate(it)
		{
			x_down_thread = new double[N];
			x_up_thread = new double[N];
			...
			while(!solution_found) {
				...
				for (int block_it = 0; block_it < number_of_blocks; block_it++) {
					...
					for (int j = 0; j < N; j++) {
						x_down_thread[j] = x_down[j];
						x_up_thread[j] = x_up[j];
					}
					for (int s = 0; s < BLOCK_SIZE(block_it*num_threads+t_id, n_blocks, M); s++) {
						scale = (b[line_down]-dotProductCSR(line_down, row_idx, cols, values, x_down_thread))/sqrNorm_line[line_down];
						index = row_idx[line_down];
						while (index < row_idx[line_down+1]) {
							col = cols[index];
							x_down_thread[col] += scale*values[index];
							index++;
						}
						line_down++;
						scale = (b[line_up]-dotProductCSR(line_up, row_idx, cols, values, x_up_thread))/sqrNorm_line[line_up];
						index = row_idx[line_up];
						while (index < row_idx[line_up+1]) {
							col = cols[index];
							x_up_thread[col] += scale*values[index];
							index++;
						}
						line_up--;
					}
					for (int i = 0; i < N; i++) {
						x_down_thread[i] -= x_down[i];
						x_up_thread[i] -= x_up[i];
					}
					#pragma omp barrier
					#pragma omp critical
						for (int i = 0; i < N; i++) {
							x_down[i] += x_down_thread[i]/num_threads;
							x_up[i] += x_up_thread[i]/num_threads;
						}
					#pragma omp barrier
					#pragma omp for
						for (int i = 0; i < N; i++) {
	...
	string filename_sol = "outputs/tomo_stop/" + matrix_type + "/CKAB_box_proj_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads) + "_" + to_string(number_of_blocks);
