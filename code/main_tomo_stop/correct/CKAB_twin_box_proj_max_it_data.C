...
// ./bin/CKAB_twin_box_proj_max_it_data.exe ct_gaussian 10 19558 16384 1 15000 5000 1 2

int main (int argc, char *argv[]) {

	if(argc != 9 && argc != 10) {
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/CKAB_twin_box_proj_max_it_data.exe <data_set> <n_runs> <M> <N> <threads> <block_size> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		while(it < max_it_stop) {
			...
			for (int i = 0; i < threads; i++) {
				...
				line = (block_begin+block_size-1)%M;
				scale = (b[line]-dotProductCSR(line, row_idx, cols, values, x_k_thread_down))/sqrNorm_line[line];
				scaleVecLine(line, row_idx, cols, values, scale, x_k_thread_down);
				for (int j = 0; j < N; j++) {
				...
				line = M-(block_begin+block_size-1)%M-1;
				scale = (b[line]-dotProductCSR(line, row_idx, cols, values, x_k_thread_up))/sqrNorm_line[line];
				scaleVecLine(line, row_idx, cols, values, scale, x_k_thread_up);
				for (int j = 0; j < N; j++) {
			...
			if ((block_begin-block_size*threads)%M+block_size*threads >= M) {
				end_matrix_counter++;
	...
	string filename_error = "errors/omp_sparse/" + matrix_type + "/CKAB_twin_box_proj_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(threads) + "_" + to_string(block_size) + "_" + to_string(max_it_stop);
	string filename_gauge = "errors/omp_sparse/" + matrix_type + "/CKAB_twin_box_proj_gauge_" + to_string(M) + "_" + to_string(N) + "_" + to_string(threads) + "_" + to_string(block_size) + "_" + to_string(max_it_stop);
