...
int main (int argc, char *argv[]) {
	...
	if(argc != 8 && argc != 9) {
		...
		cout << "'./bin/RKA_box_proj_alpha_stop.exe <data_set> <n_runs> <M> <N> <it_per_thread> <bucket_size> <alpha>'" << endl;
		exit(1);
	}
	...
	if (argc == 8 && matrix_type.compare("ct") == 0) {
		...
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/RKA_box_proj_stop.exe <data_set> <n_runs> <M> <N> <it_per_thread> <bucket_size> <alpha>'" << endl;
		exit(1);
	}
