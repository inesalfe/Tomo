...
int main (int argc, char *argv[]) {
	...
	if(argc != 8 && argc != 9) {
		...
		cout << "'./bin/SRKAWOR_alpha_stop.exe <data_set> <n_runs> <M> <N> <it_per_thread> <bucket_size> <alpha>'" << endl;
		exit(1);
	}
	...
	string filename_sol = "outputs/tomo_stop/" + matrix_type + "/SRKAWOR_alpha_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + str_alpha;
