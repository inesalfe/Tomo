...
int main (int argc, char *argv[]) {
	...
	if(argc != 8 && argc != 9) {
		...
		cout << "'./bin/SRKBWOR_box_proj_parallel_stop_par.exe <data_set> <n_runs> <M> <N> <number_of_blocks> <min_it> <bucket_size>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		it = 0;
		start = omp_get_wtime();
		#pragma omp parallel private(line_down_idx, line_up_idx, line_down, line_up, scale, t_id, diff_entry) firstprivate(it)
		{
