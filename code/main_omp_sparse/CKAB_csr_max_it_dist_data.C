...
int main (int argc, char *argv[]) {
	...
	if(argc != 9 && argc != 10) {
		...
		cout << "'./bin/CKAB_csr_max_it_dist_data.exe <data_set> <n_runs> <M> <N> <threads> <block_size> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		while(it < max_it_stop) {
			...
			for (int i = 0; i < threads; i++) {
				line = BLOCK_LOW(i, threads, M) + (it*block_size)%BLOCK_SIZE(i, threads, M);
				...
				for (int k = 1; k < block_size-1; k++) {
					line = BLOCK_LOW(i, threads, M) + (it*block_size + k)%BLOCK_SIZE(i, threads, M);
				...
				line = BLOCK_LOW(i, threads, M) + (it*block_size + block_size-1)%BLOCK_SIZE(i, threads, M);
				scale = (b[line]-dotProductCSR(line, row_idx, cols, values, x_k_thread))/sqrNorm_line[line];
				scaleVecLine(line, row_idx, cols, values, scale, x_k_thread);
				for (int j = 0; j < N; j++) {
	...
	string filename_error = "errors/omp_sparse/" + matrix_type + "/CKAB_csr_dist_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(threads) + "_" + to_string(block_size) + "_" + to_string(max_it_stop);
	string filename_res = "errors/omp_sparse/" + matrix_type + "/CKAB_csr_dist_res_" + to_string(M) + "_" + to_string(N) + "_" + to_string(threads) + "_" + to_string(block_size) + "_" + to_string(max_it_stop);
