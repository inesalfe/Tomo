...
int main (int argc, char *argv[]) {
	...
	if(argc != 8 && argc != 9) {
		...
		cout << "'./bin/RKAB_csr_parallel_max_it_data.exe <data_set> <n_runs> <M> <N> <block_size> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		#pragma omp parallel private(line, scale, t_id, x_k_thread) firstprivate(it)
		{
			...
			while(it < max_it_stop) {
				it++;
				#pragma omp barrier
				// printf("%d %.15f %.15f\n", t_id, x_k[0], x_k[52]); fflush(stdout);
				line = dist(generator);
	...
	string filename_error = "errors/omp_sparse/" + matrix_type + "/RKAB_csr_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads) + "_" + to_string(block_size) + "_" + to_string(max_it_stop);
	string filename_res = "errors/omp_sparse/" + matrix_type + "/RKAB_csr_res_" + to_string(M) + "_" + to_string(N) + "_" + to_string(num_threads) + "_" + to_string(block_size) + "_" + to_string(max_it_stop);
