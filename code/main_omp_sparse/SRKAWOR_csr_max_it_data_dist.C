...
int main (int argc, char *argv[]) {
	...
	if(argc != 8 && argc != 9) {
		...
		cout << "'./bin/SRKAWOR_csr_max_it_data_dist.exe <data_set> <n_runs> <M> <N> <it_per_thread> <max_it_stop> <step_save>'" << endl;
		exit(1);
	}
	...
	string filename_error = "errors/omp_sparse/SRKAWOR_csr_error_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + ".txt";
	string filename_res = "errors/omp_sparse/SRKAWOR_csr_res_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + ".txt";
