...
int main (int argc, char *argv[]) {
	...
	if(argc != 8 && argc != 9) {
		...
		cout << "'./bin/CKA_csr_max_it_alpha_sol.exe <data_set> <n_runs> <M> <N> <it_per_thread> <alpha> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	string filename_sol = "outputs/omp_sparse/" + matrix_type + "/CKA_alpha_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(it_per_thread) + "_" + to_string(alpha) + "_" + to_string(max_it_stop);
