...
int main (int argc, char *argv[]) {
	...
	if(argc != 9) {
		...
		cout << "'./bin/RKAB_single_csr_seq_uni_newSC.exe <data_set> <n_runs> <eps> <M> <N> <threads> <it_per_thread> <max_it_stop>'" << endl;
		exit(1);
	}
		...
		while(!solution_found) {
			...
			for (int i = 0; i < threads; i++) {
				...
				for (int k = 0; k < it_per_thread; k++) {
					line = BLOCK_LOW(i, threads, M) + (it*it_per_thread + k)%BLOCK_SIZE(i, threads, M);
