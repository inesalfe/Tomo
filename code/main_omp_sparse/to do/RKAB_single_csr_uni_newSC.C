...
int main (int argc, char *argv[]) {
	...
	if(argc != 8) {
		...
		cout << "'./bin/RKAB_single_csr_uni_newSC.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		#pragma omp parallel private(line, scale, t_id, x_k_thread) firstprivate(it)
		{
			...
			while(!solution_found) {
				it++;
				#pragma omp barrier
				line = BLOCK_LOW(t_id, num_threads, M) + (it*it_per_thread)%BLOCK_SIZE(t_id, num_threads, M);
				scale = (b_shuffle[line]-dotProductCSR(line, row_idx_shuffle, cols_shuffle, values_shuffle, x_k))/sqrNorm_line_shuffle[line];
				scaleNewVecLine(line, row_idx_shuffle, cols_shuffle, values_shuffle, scale, x_k, x_k_thread);
				for (int i = 1; i < it_per_thread; i++) {
					line = BLOCK_LOW(t_id, num_threads, M) + (it*it_per_thread + i)%BLOCK_SIZE(t_id, num_threads, M);
					scale = (b_shuffle[line]-dotProductCSR(line, row_idx_shuffle, cols_shuffle, values_shuffle, x_k_thread))/sqrNorm_line_shuffle[line];
					scaleVecLine(line, row_idx_shuffle, cols_shuffle, values_shuffle, scale, x_k_thread);
				}
				for (int i = 0; i < N; i++) {
					x_k_thread[i] -= x_k[i];
				}
