...
int main (int argc, char *argv[]) {
	...
	if(argc != 7) {
		...
		cout << "'./bin/RKA_csr_uni_newSC.exe <data_set> <n_runs> <eps> <M> <N> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	if (matrix_type.compare("dense") == 0) {
		...
		cout << "Incorrect number of arguments: Corret usage is ";
		cout << "'./bin/RKA_csr_uni_newSC.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread>'" << endl;
		exit(1);
	}
	...
	for (int i = 0; i < M; i++) {
		sqrNorm_line[i] = num_threads*sqrNormRow(i, row_idx, cols, values_csr);
		if (sqrNorm_line[i] == 0) {
