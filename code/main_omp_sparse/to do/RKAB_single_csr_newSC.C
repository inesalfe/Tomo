...
int main (int argc, char *argv[]) {
	...
	if(argc != 8) {
		...
		cout << "'./bin/RKAB_single_csr_newSC.exe <data_set> <n_runs> <eps> <M> <N> <it_per_thread> <max_it_stop>'" << endl;
		exit(1);
	}
	...
	for(int run = 0; run < n_runs; run++) {
		...
		#pragma omp parallel private(line, scale, t_id, x_k_thread) firstprivate(it)
		{
			...
			while(!solution_found) {
				...
				scaleNewVecLine(line, row_idx, cols, values_csr, scale, x_k, x_k_thread);
				for (int i = 0; i < it_per_thread-1; i++) {
					line = dist(generator);
