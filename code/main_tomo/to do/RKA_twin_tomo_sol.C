...
int main (int argc, char *argv[]) {
	...
	if(argc != 7) {
		...
		cout << "'./bin/RKA_twin_tomo_sol.exe <data_set> <M> <M> <seed> <max_it_stop> <threads>'" << endl;
		exit(1);
	}
	...
	string filename_sol = output_foler + "RKA_twin_sol_" + to_string(M) + "_" + to_string(N) + "_" + to_string(seed) + "_" + to_string(threads) + "_" + to_string(max_it_stop) + ".txt";
